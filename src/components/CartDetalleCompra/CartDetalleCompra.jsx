...
export default function CartDetalleCompra() {
  ...
  for (let iterador in cart) {
    totalCantidadProductos +=
      cart[iterador].quantity * cart[iterador].price.slice(1);
  }
  ...
  return (
    ...
    >
      ...
      <div>
        <button class="btnDelete" onClick={() => clear()}>
          {" "}
          Vaciar carrito{" "}
        </button>
      </div>
      <div className="detalleCompra">
        {cart.map((producto) => (
          <div>
            ...
            <button class="btn" onClick={() => removeItems(producto.id)}>
              Eliminar
            </button>
          </div>
        ))}
      ...
      <UserForm cart={cart} cantidad={totalCantidadProductos} />
    </div>
  );
}
